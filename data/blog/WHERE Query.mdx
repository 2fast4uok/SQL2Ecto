...
In [Ecto.Query](https://hexdocs.pm/ecto/Ecto.Query.html) `where/3` is used to fetch records based on the particular value.
...
`where([c], c.salary == 9000)` -> Here `[c]` is the reference variable which is used to refer to `HR.Employee`. This query will fetch the `HR.Employee` struct which `salary` is equal to 9000.  Refer <a href="/blog/Aliases in Ecto">Aliases in Ecto</a> to know about aliases/reference variables.
