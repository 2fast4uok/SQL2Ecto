...
- `join(:left, [p], r in HR.Region, on: p.region_id == r.region_id)` -> Left join, joining rows whose `region_id` is the same. Here `p` and `r` are reference variables. Refer to <a href="/blog/Aliases in Ecto">Aliases in Ecto</a> to understand reference variables.
