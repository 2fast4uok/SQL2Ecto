...
The `c` in both condition is reference variable which is referring to the struct `HR.Employee`. To know about reference variable refer <a href="/blog/Aliases in Ecto">reference variable</a>.
...
The above result shows `employees` whose `phone_number` is null and `first_name` starts with J or j.
...
`where` is a keyword. `is_nil(c.phone_number)` and `ilike(c.first_name, "J%")` is the value.
