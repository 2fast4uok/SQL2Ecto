...
The `join/5` function is used to perform a `right join` in Ecto. For more information about the `join/5` function, visit my [[08. join]] page. Now letskk's explore the `:right` option in join.
